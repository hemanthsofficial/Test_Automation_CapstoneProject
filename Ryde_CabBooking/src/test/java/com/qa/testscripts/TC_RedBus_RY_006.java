...
public class TC_RedBus_RY_006 extends TC_RedBus_RY_002 {
	@Test(dependsOnMethods = "main2",dataProvider="getData")
	public void main6(String destination) throws InterruptedException, IOException {
		...
		if (verifycabwin) {
			...
			captureScreenshot(driver, "main6");
			Reporter.log("\nCab booking window is not opened", true);
			sAssert.assertTrue(verifycabwin);
		}

		// Type input data on destination box
		ryde.getdestinationbox().click();
		ryde.getdestinationinput().sendKeys(destination);
		Thread.sleep(3000);

		// Click on the expected result
		List<WebElement> select = ryde.getdestinationselect();
		int len = 0;
		len = select.size();
		if (len != 0) {
			for (int i = 0; i < len; i++) {
				if (select.get(i).getText().contains(destination)) {
					select.get(i).click();
					Thread.sleep(2000);
					...
					Reporter.log("\nUser input for destination location is accepted", true);
					break;
				}
			}
		} else {
			...
			if (ryde.getwrong().isDisplayed()) {
				captureScreenshot(driver, "main6");
				Reporter.log("\nInvalid destination location entered, user input not accepted", true);
